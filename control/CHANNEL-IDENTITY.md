...
Channel preferences resolve the verified role using the existing `verification_role` setting (a permanent role ID), never its display name. An unset or deleted configured role fails closed. Renaming the verified role does not require a deployment.
