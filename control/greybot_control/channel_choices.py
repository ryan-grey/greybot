...
def choices(guild, roles, channels, member, owned):
    ...
    for channel in channels:
        if channel.get("type") not in (0, 2, 5, 13) or channel["id"] == guild.get("rules_channel_id") or channel.get("name") in ("bots", "channel-preferences", "channel-list", "channel-guide", "verify-membership"):
            continue
